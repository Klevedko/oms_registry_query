-- select * from pd_query where query_id = 'F003(фд)/Лицензии медицинских организаций действующие на текущую дату'
-- oracle
           SELECT  count(*)
             FROM  OMS.DT_MEDLIC 
                ,  OMS.DT_MEDCOM 
            WHERE  OMS.DT_MEDCOM .RECORD_IN             (+)  =  OMS.DT_MEDLIC .RECORD_IN_MEDCOM  
              ...
              AND  OMS.DT_MEDLIC .LIC_DATE_START            <=  SYSDATE
              AND  NVL (         
                   OMS.DT_MEDLIC .LIC_DATE_END , SYSDATE)   >=  SYSDATE
              AND  NVL (         
                   OMS.DT_MEDLIC .LIC_DATE_TERM, SYSDATE)   >=  SYSDATE
           ...
           SELECT  count(*)
             FROM  OMS.DT_MEDLIC 
              ...
              AND  OMS.DT_MEDCOM .edate              >=  now()
							AND  OMS.DT_MEDLIC .LIC_DATE_START            <=  now()
              where  COALESCE (         
                   OMS.DT_MEDLIC .LIC_DATE_END , now())   >=  now()
              AND  COALESCE (         
                   OMS.DT_MEDLIC .LIC_DATE_TERM, now())   >=  now()
  /* +Filter (AND  OMS.DT_MEDLIC .RECORD_IN_MEDCOM        JOIN  OMS.DT_MEDCOM .RECORD_IN                                                  
                                                       BROWSER  F003(фм)/Медицинские организации                                           
                                                         QUERY  F003(фм)/Медицинские организации                                          ) */
  /* +Filter (AND  OMS.DT_MEDCOM .RECORD_IN_TSFOMS        JOIN  OMS.DT_TSFOMS .RECORD_IN                                                  
