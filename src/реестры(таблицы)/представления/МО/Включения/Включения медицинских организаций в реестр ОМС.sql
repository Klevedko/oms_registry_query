-- select * from pd_query where query_id = 'F003(фд)/Включения медицинских организаций в реестр ОМС'

- oracle 
           SELECT  count(*)
             FROM  OMS.DT_MEDINC 
                ,  OMS.DT_MEDCOM 
            WHERE  OMS.DT_MEDCOM .RECORD_IN             (+)  =  OMS.DT_MEDINC .RECORD_IN_MEDCOM        
              AND  OMS.DT_MEDCOM .DATE_START            (+) <=  SYSDATE
              AND  OMS.DT_MEDCOM .DATE_END              (+) >=  SYSDATE
  /* +Filter (AND  OMS.DT_MEDINC .RECORD_IN_MEDCOM        JOIN  OMS.DT_MEDCOM .RECORD_IN                                                  
...
-- posgtresql
           SELECT  count(*)
             FROM  OMS.DT_MEDINC 
                left join  OMS.DT_MEDCOM on OMS.DT_MEDCOM .temp_id              =  OMS.DT_MEDINC .dt_medcom_id
